...
public class ButtonManager : MonoBehaviour
{
    ...
    public void ToggleObject(GameObject toToggle)
    {
        bool isActive = (toToggle.transform.position == enabledPos.transform.position);

        if (isActive)
        {
            DisableObject(toToggle);
        }
        else
        {
            EnableObject(toToggle);
        }
    }
}
